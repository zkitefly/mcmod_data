    ...
    if file.endswith('.html'):
        ...
        with open(os.path.join(mcmodwebdata_path, file), 'r', encoding='utf-8') as f:
            ...
            for script in soup.find_all('script'):
                if script.string and 'www.mcbbs.net/thread-' in script.string:
                    MCBBS = script.string.split('www.mcbbs.net/thread-')[1].split('-')[0]
                    break

            MRID = None
            for script in soup.find_all('script'):
                if script.string and '.val("https://modrinth.com/mod/' in script.string:
                    MRID = script.string.split('.val("https://modrinth.com/mod/')[1].split('"')[0]
                    break
            ...
            if CFID is not None:
                links_list.append({
                    'type': 'curseforge',
                    'url': f'https://www.curseforge.com/minecraft/mc-mods/{CFID}'
                })
            links_list.append({
                'type': 'mcmod',
                'url': f'https://www.mcmod.cn/class/{NAME}.html'
            })
            ...
            if MRID is not None:
                links_list.append({
                    'type': 'modrinth',
                    'url': f'https://www.modrinth.com/mod/{MRID}'
                })
